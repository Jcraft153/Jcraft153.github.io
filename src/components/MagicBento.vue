...
<style>
.magic-bento { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; }.bento-card { --glow-x: 50%; --glow-y: 50%; position: relative; min-height: 180px; padding: 1.5rem; overflow: hidden; border: 1px solid rgba(235, 231, 239, .38); border-radius: 18px; background: rgba(66, 23, 97, .82); box-shadow: 0 14px 45px rgba(24, 5, 42, .34); color: #f0eff3; transition: transform 180ms ease, border-color 180ms ease, box-shadow 180ms ease; transform-style: preserve-3d; }.bento-card::before { position: absolute; inset: -1px; content: ''; pointer-events: none; background: radial-gradient(280px circle at var(--glow-x) var(--glow-y), rgba(212, 163, 250, .32), transparent 62%); opacity: .9; }.bento-card:hover { border-color: rgba(249, 247, 251, .9); box-shadow: 0 18px 58px rgba(24, 5, 42, .5), 0 0 26px rgba(208, 143, 255, .35); }.bento-card > * { position: relative; }.bento-card--intro { grid-column: span 1; }.bento-card--readme { grid-column: span 3; min-height: 280px; }.bento-card--link { display: flex; flex-direction: column; justify-content: space-between; grid-column: span 4; min-height: 110px; color: #f4f3f6; text-decoration: none; }.bento-card--link strong { font-size: clamp(1.45rem, 3vw, 2.1rem); }.bento-card h2 { margin: .35rem 0; font-size: 2rem; }.bento-card p { margin: 0; line-height: 1.6; }.bento-label { color: #dfc5f5; font-size: .75rem; font-weight: 800; letter-spacing: .12em; text-transform: uppercase; }@media (max-width: 620px) { .magic-bento { grid-template-columns: 1fr; }.bento-card--intro, .bento-card--readme, .bento-card--link { grid-column: span 1; }.bento-card--readme { min-height: 0; } }
</style>
